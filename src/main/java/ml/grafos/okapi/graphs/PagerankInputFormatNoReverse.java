...
public class PagerankInputFormatNoReverse extends TextEdgeInputFormat<IntWritable, IntWritable> {

    private static final Logger INPUT_FORMAT_LOGGER = LoggerFactory.getLogger(PagerankInputFormatNoReverse.class);
    // CSV split regex
    //private static final Pattern SEPARATOR = Pattern.compile(",(?=([^\"]*\"[^\"]*\")*(?![^\"]*\"))");
    private static final Pattern SEPARATOR = Pattern.compile(",");

    @Override
    public EdgeReader<IntWritable, IntWritable> createEdgeReader(InputSplit inputSplit, TaskAttemptContext taskAttemptContext) throws IOException {

        EdgeReader<IntWritable, IntWritable> edgeReader = new IntIDReader();
